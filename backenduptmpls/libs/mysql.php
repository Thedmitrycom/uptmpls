...
class uptimePlus
{
	...
	public function __construct($wl_db_host, $wl_db_user, $wl_db_pass, $opt = array()) {
		$this->wl_db_host = $wl_db_host;
		$this->wl_db_user = $wl_db_user;
		$this->wl_db_pass = $wl_db_pass;

		$this->defaults = array(
			'host' => $this->wl_db_host,
			'user' => $this->wl_db_user,
			'pass' => $this->wl_db_pass,
			'db' => 'uptimePlus',
			'port' => 3306,
			'socket' => null,
			'pconnect' => false,
		...
		{ 
			...
			$arrayError['message'] = 'serverdown1';
	...
	public function getRow()
	{
		...
		if (strpos($query, 'limit ') !== false || strpos($query, 'LIMIT ') !== false) {} else {
			$query = $query.' LIMIT 1';
		}
		if ($res = $this->rawQuery($query)) {
			$ret = $this->fetch($res);
	...
	protected function rawQuery($query)
	{
		...
		// $text .= $query."\n";
		// $filename = $_SERVER['DOCUMENT_ROOT'].'/debug_mysql.log';
		// $fh = fopen($filename, "a");
		// fwrite($fh, $text);
		// fclose($fh);
